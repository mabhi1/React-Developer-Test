...
const inputVariants = cva(
  "border-2 focus-visible:outline-none bg-slate-50 border-slate-400 rounded",
  {
    variants: {
      variant: {
      ...
      wide: {
        ...
        sm: "w-20 lg:w-40 md:w-60",
        md: "w-40 lg:w-60 md:w-80",
...
const Input: FC<InputProps> = ({ className, wide, variant, ...props }) => {
  ...
  return (
    <input
      className={mergeClass(inputVariants({ className, variant, wide }))}
      {...props}
    />
  );
};
